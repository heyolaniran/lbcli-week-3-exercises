# Create a 1-of-4 P2SH multisig address from the public keys in the four inputs of this raw tx:
TX=""

SIGNATURES=$(bitcoin-cli -regtest decoderawtransaction $TX | jq -r '([.vin[].txinwitness[1]]) | join(",")')

P2SH_DESCRIPTOR=$(bitcoin-cli -regtest getdescriptorinfo "sh(multi(1,$SIGNATURES))" | jq -r ".descriptor")
P2SH_ADDR=$(bitcoin-cli -regtest deriveaddresses "$P2SH_DESCRIPTOR" | jq -r ".[0]")


# RIGHT ONE in this case

SIGNS=$(bitcoin-cli -regtest decoderawtransaction $TX | jq -r '[.vin[].txinwitness[1]]')

P2SH_CMD=$(bitcoin-cli -regtest createmultisig 1 "$SIGNS" | jq -r ".address")

echo $P2SH_CMD
